...
app.use((req,res,next)=>{
  const protectedAdminPages = new Set([
    '/panel-admin.html',
    '/panel-competiciones.html',
    '/panel-copas.html',
    '/panel-resultados.html',
    '/panel-noticias.html'
  ]);
  if(protectedAdminPages.has(req.path) && !telHasAdminSession(req)){
    return res.redirect('/?admin_login=1#login');
  }
  next();
});

app.use(express.static(__dirname, telStaticOptions));
...
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || process.env.WEB_ADMIN_PASSWORD || 'admin123';
...
app.post('/api/admin/final-login', express.json(), (req,res)=>{
  ...
  const adminPassword = process.env.ADMIN_PASSWORD || process.env.WEB_ADMIN_PASSWORD || 'admin123';
...
function telDirectPasswordOk(req){
  ...
  return pass === String(process.env.ADMIN_PASSWORD || process.env.WEB_ADMIN_PASSWORD || 'admin123') || (req.session && req.session.isAdmin === true);
}
